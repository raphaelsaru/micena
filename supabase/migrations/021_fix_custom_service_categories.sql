-- Corrigir funções RPC para categorias personalizadas com melhor tratamento de erros e uppercase
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM custom_service_categories WHERE UPPER(name) = normalized_name) THEN
...
BEGIN
    ...
    IF category_name IS NOT NULL THEN
        ...
        IF EXISTS (
            ...
            WHERE UPPER(name) = normalized_name AND id != category_id
        ) THEN
            RAISE EXCEPTION 'Já existe uma categoria com o nome "%"', normalized_name;
        END IF;
    ELSE
...
-- Normalizar categorias existentes para uppercase
UPDATE custom_service_categories 
